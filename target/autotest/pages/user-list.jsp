    ...
    <script>
        $(document).ready(function () {
            ...
            $.ajax({
                type:"POST",
                dataType:"json",
                url:"/user/findAllUserPage?size=10&page=1",
                ...
                success:function (data) {
                    ...
                    $('#totalNum').text("总共的页数："+pageNum+"；总共的用户数："+totalNum)
                    ...
                    for (var i = 0; i <data.result.length; i++) {
                        var $tr=$("<tr>"+"</tr>");
                        var baseURI="${pageContext.request.contextPath}/user/findUserById?userid="
                        ...
                        $tr.append("<td>"+"<div class=\"layui-unselect layui-form-checkbox\" lay-skin=\"primary\">"+"<i class=\"layui-icon layui-icon-ok\">"+"</i>"+"</div>"+"</td>"+"<td>"+data.result[i].userid+"</td>"+"<td>"+data.result[i].username+"</td>"+"<td>"+data.result[i].password+"</td>"+"<td>"+data.result[i].email+"</td>"+"<td>"+data.result[i].phoneNum+"</td>"+"<td>"+data.result[i].joinTime+"</td>"+"<td class='"+statusStrCss+"' style='text-decoration: none;text-align:center'>"+data.result[i].statusStr+"</td>"+"<td>"+"<a title='编辑' id='"+userid+"' href='"+baseURI+""+userid+"' >"+"<i class=\"layui-icon\">&#xe642;</i>"+"</a>"+"<a title='删除' id='"+userid+"' onclick='delCase(this.id)'>"+"<i class=\"layui-icon\">&#xe640;</i>"+"</a>"+"</td>")
                    }
                }
            })
        })
        ...
        function clickPage(page,size) {
            ...
            $.ajax({
                type:"POST",
                dataType:"json",
                url:"/user/findAllUserPage?size="+size+"&page="+page,
                contentType:"application/json;charset=utf-8",
                success:function (data) {
